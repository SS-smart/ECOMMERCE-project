          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(spuSaleAttr,index) in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                :key="spuSaleAttrValue.id" @click="changeActive(spuSaleAttrValue,spuSaleAttr.spuSaleAttrValueList)">{{spuSaleAttrValue.saleAttrValueName}}</dd>             
              </dl>
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- //这里在加入购物车，进行路由跳转之前，发请求把你购买的产品的信息通过请求的形式通知服务器，服务器进行相应的存储 -->
...
<script>
  ...
  import {mapGetters,skuInfo} from 'vuex'
  export default {
    name: 'Detail',
    data(){
      return {
    ...
    methods:{
      ...
      //排他思想，遍历全部售卖属性值isChecked为零没有高亮了
      arr.forEach(item => {
        item.isChecked = 0
      });
      ...
      async addShopCar(){
        try {
          ...
          //进行路由跳转，将产品的信息传递过去
          //一些简单的数据通过querry传过去
          //比较复杂的数据，通过会话存储（不持久化，会话结束数据消失）会话存储和本地存储一般存储的是字符串
